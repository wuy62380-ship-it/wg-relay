#!/bin/bash
# ==========================================
# WireGuard 智能中转部署脚本 v131.0 (终极安装修复版)
# 找回完整的 XanMod 智能安装逻辑
# ==========================================
...
xanmod_add_repo() {
    ...
    echo -e "${GREEN}  - 仓库添加成功 (Codename: $os_codename)${NC}"
}
...
EOF
                    ...
                    echo -e "${RED}✘ 找不到适合的内核包，自动回退普通调优${NC}"
                fi
            else
                echo -e "${YELLOW}✘ 仓库添加失败，自动回退普通 BBR 调优${NC}"
            fi
            tune_choice=1
            ;;
        *)
...
add_relay_port() {
    ...
    while true; do 
        ...
        if flock -s 200 -c "grep -q '^${MAP_PORT}|' '${NODES_INFO}'" 200>"$LOCK_FILE" 2>/dev/null; then
...
while true; do
    clear
    echo -e "${CYAN}╔════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  WG 智能中转 v131.0 (终极安装修复版)      ║${NC}"
    echo -e "${CYAN}╠════════════════════════════════════════════╣${NC}"
