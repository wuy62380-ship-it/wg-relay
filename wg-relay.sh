...
WG_PORT="51820"
SYSCTL_FILE="/etc/sysctl.d/99-yg-optimize.conf"
...
check_system() {
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        if [ "$ID" != "debian" ] && [ "$ID" != "ubuntu" ]; then echo -e "${RED}仅支持 Debian/Ubuntu${NC}"; exit 1; fi
    else echo -e "${RED}无法识别系统${NC}"; exit 1; fi
}

clear_stdin() { while read -t 0.1 -n 1 dummy; do :; done; }
pause_return() { clear_stdin; echo -e "${YELLOW}按 Enter 键返回主菜单...${NC}"; read -r; }
kill_apt_locks() { rm -f /var/lib/dpkg/lock /var/lib/dpkg/lock-frontend /var/cache/apt/archives/lock /var/lib/apt/lists/lock 2>/dev/null; dpkg --configure -a 2>/dev/null; }
...
force_sync_time() {
    ...
    local y=$(date +%Y)
    if [ "$y" -lt 2020 ] || [ "$y" -gt 2030 ]; then
        local t=$(curl -m 5 -sI https://www.cloudflare.com 2>/dev/null | grep -i '^date:' | sed 's/^[Dd]ate: //g' | tr -d '\r')
        [ -n "$t" ] && date -s "$t" >/dev/null 2>&1
    fi
}
...
tune_system() {
    ...
    if uname -r | grep -qi "xanmod"; then
        echo -e "${GREEN}✓ 已是 XanMod 内核，应用调优...${NC}"
        echo "net.core.default_qdisc=fq_pie" > $SYSCTL_FILE
        echo "net.ipv4.tcp_congestion_control=bbr" >> $SYSCTL_FILE
        sysctl -p $SYSCTL_FILE > /dev/null 2>&1
    else
        echo -e "${YELLOW}应用普通 BBR 调优...${NC}"
...
init_relay() {
    ...
    [ -f "$WG_CONF" ] && { read -p "${RED}已有配置将被覆盖！确定？[y/N]: ${NC}" c; [[ ! "$c" =~ ^[Yy]$ ]] && { pause_return; return; }; }
...
gen_landing_code() {
    ...
    while true; do read -p "节点名称: " NODE_NAME; [ -n "$NODE_NAME" ] && break; echo -e "${RED}不能为空${NC}"; done
    MAX_IP=1
    ...
    while true; do read -p "客户端端口: " MAP_PORT; [[ "$MAP_PORT" =~ ^[0-9]+$ ]] && [ "$MAP_PORT" -ge 1 ] && [ "$MAP_PORT" -le 65535 ] && [ "$MAP_PORT" != "$WG_PORT" ] && break; echo -e "${RED}端口无效${NC}"; done
...
deploy_landing() {
    ...
    read -p "落地机监听端口 (默认 443): " LAND_PORT
...
EOF
    ...
    if [ -z "$SB_PUB" ] || [ -z "$UUID" ] || [ -z "$SHORT_ID" ]; then
        echo -e "${RED}❌ 致命错误：Sing-box 生成密钥失败！可能程序损坏。${NC}"
        pause_return; return
    fi
...
list_relay_nodes() {
    ...
    [ ! -f "$NODES_INFO" ] && { echo -e "${RED}暂无节点${NC}"; pause_return; return; }
...
list_landing_nodes() {
    ...
    [ ! -f "$LAND_INFO" ] && { echo -e "${RED}无记录${NC}"; pause_return; return; }
